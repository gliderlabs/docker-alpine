...
@test "repository list is correct" {
  ...
  [ $status -eq 0 ]
  [ "${lines[0]}" = "https://dl-cdn.alpinelinux.org/alpine/v3.3/main" ]
  [ "${lines[1]}" = "https://dl-cdn.alpinelinux.org/alpine/v3.3/community" ]
  [ "${lines[2]}" = "" ]
}
...
@test "CVE-2016-2183, CVE-2016-6304, CVE-2016-6306" {
  run docker run alpine:3.3 sh -c 'apk version -t $(apk info -v | grep ^libssl | cut -d- -f2-) 1.0.2i-r0 | grep -q "[=>]"'
  [ $status -eq 0 ]
}

@test "CVE-2016-7052" {
  run docker run alpine:3.3 sh -c 'apk version -t $(apk info -v | grep ^libssl | cut -d- -f2-) 1.0.2j-r0 | grep -q "[=>]"'
  [ $status -eq 0 ]
}
